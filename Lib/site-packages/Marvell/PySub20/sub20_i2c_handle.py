...
from Marvell.PySub20.Common.Constants import NULL
...
class Sub20Handle(object):
    ...
    def Init_Sub_Device(self, lptNum):

        if lptNum == NULL:
            self.sub_dev = NULL
        else:
            self.sub_dev = self.FindConnectedDevice(lptNum)
            if self.sub_dev == NULL:
    ...
    def SetI2CConfigParams(self, lptNum, freq, waitTime, i2cMode, i2cInterface, errMsg=''):
        ...
        #         msg = "Sub20, serial number= " + toHexStr(lptNum) + ", could not be found."
        #         self.raiseSub20RuntimeError(msg)
        self.Init_Sub_Device(lptNum)

        self.openSub20()

        if self.sub_hndl == NULL:
    ...
    def subI2CRead(self, addr, ma, ma_sz, nBytes) -> list:
        ...
        try:
            ...
            if retVal != 0:
                msg = f"sub_i2c_read failed to read data from address {toHexStr(addr)}"
                self.raiseSub20RuntimeError(msg)

            if self.log_level == 1:
                msg = f"i2c read slaveAddr={toHexStr(addr)}, ma={hex(ma)}, ma_sz={ma_sz}, datta={[hex(d) for d in list(data.raw)]} nBytes={nBytes} "
    ...
    def subI2CWrite(self, addr: int, ma, ma_sz, data: list, nBytes: int):
        """
        important: If ma_sz is zero "Memory Address Write" stage will be skipped.
        """
        self.openSub20()
        try:

            dataToWrite = bytes(data) #create_string_buffer(data)
            retVal = self.sub_i2c_write(self.sub_hndl, addr, ma, ma_sz, dataToWrite, nBytes)

            if retVal != 0:
                msg = f"sub_i2c_write failed to write data to address {toHexStr(addr)}"
                self.raiseSub20RuntimeError(msg)

            if self.log_level == 1:
                msg = f"i2c write slaveAddr={toHexStr(addr)}, ma={hex(ma)}, ma_sz={ma_sz}, datta={[hex(d) for d in data]} nBytes={nBytes}"
                print(msg)
    ...
    def subI2CWrite_old(self, addr: int, nBytes: int, data: bytes):
        """
        important: If ma_sz is zero "Memory Address Write" stage will be skipped.
        """
        self.openSub20()
