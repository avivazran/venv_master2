...
class PyCommunicationAlgo(ABCCommunicationAlgo):
    ...
    def Connect(self, channeltype, portId, baudRate=115200, parity="N", dataBits=8, stopBits=1,
                ipAddr=None, uname=None, password=None, delayBetweenCharsWrite=-1, client_ip="0.0.0.0", **extraParameters):
        try:
            self.m_connAlias, self.m_channel = ChannelFactory.Create(channeltype, portId, ipAddr,
                                                                     self.m_prompt, uname, password, baudRate,
                                                                     delayBetweenCharsWrite, **extraParameters)

            self.m_channel.Connect()
            self.m_IsConnected = True
            self.m_channelDataReader = ChannelReader(self.m_channel)
            self.m_client_ip = client_ip
            return self.m_connAlias
        except ConnectionFailedException as e:
            raise
        except Exception as e:
            raise ConnectionFailedException(channeltype.name, "{} Connection Failed!!\n".format(channeltype.name) + str(e))
            # return None  --> Uncreachble Code

    def getIpOfConnectedClient(self):
        return self.m_client_ip

    def forceDisconnect(self, client_ip="0.0.0.0"):
        self.m_channelDataReader.emitForceDisconnect(client_ip)
        ...
        try:
            while (True):
                ...
                for key, pattern in iteritems(dicPattern):
                    bytes_pattern = pattern if isinstance(pattern, bytes) else pattern.encode('utf-8')
                    match = re.search(bytes_pattern, output, re.MULTILINE)
                    if match:
                        return output
                ...
                if exe_time >= timeOutSeconds:
                    raise Exception("Can't read till any prompt, timeout exception " + str(exe_time) +
                                    " seconds. \n" + "Current buffer: " + output.decode('utf-8'))
                time.sleep(0.1)
