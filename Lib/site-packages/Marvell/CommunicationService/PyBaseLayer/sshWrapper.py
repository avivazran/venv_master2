...
class sshWrapper(object):
    ...
    def open(self):
        ...
        try:
            ...
            # handle case where no authentication is required on server
            tp: Optional[paramiko.Transport]
            if (tp := self._client.get_transport()) is not None:
                tp.auth_none(self._username)
                self._shell = self._client.invoke_shell(width=self._shell_width,
                                                        height=self._shell_height)  # self.print_data()
                self._login_with_none_authentication()
    ...
    def _login_with_none_authentication(self):
        self.read_until(b"User Name:")
        self.write(self._username + b'\n')
        self.read_until(b"Password:")
        self.write(self._password + b'\n')
