...
from Marvell.pytoolsinfra.ConfigurationModule.Configurator.Providers.ExcelWrapper import *
...
class TestExcelWrapper(TestCase):
    ...
    def test_set_cell_scientific_format(self):
        ret = self.wb.CreateSheet(self.sheet_name)
        self.assertTrue(ret)
        set_val = "0.00003"
        self.wb.setCellVal(self.sheet_name, 1, 2, set_val)

        ret = self.wb.setCellFormatScientific(self.sheet_name, 1, 2, 2)
        self.assertTrue(ret)
    ...
    def test_set_cell_alignment(self):
        ...
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Left)
        self.assertTrue(ret)
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Right)
        self.assertTrue(ret)
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Center)
        self.assertTrue(ret)
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Left, VAlign.Center)
        self.assertTrue(ret)
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Left, VAlign.Top)
        self.assertTrue(ret)
        ret = self.wb.setCellAlignment(self.sheet_name, 1, 2, HAlign.Left, VAlign.Bottom)
        self.assertTrue(ret)
